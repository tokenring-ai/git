...
export async function execute(args, registry) {
 ...
 try {
  const { stdout: statusOutput } = await fileSystem.executeCommand(["git", "status", "--porcelain"]);
  if (statusOutput.trim()) {
   chatService.errorLine("There are uncommitted changes. Please commit or stash your changes before rollback.");
 ...
 try {
  ...
  if (args.commit) {
   ...
   await fileSystem.executeCommand(["git", "reset", "--hard", `HEAD~${args.steps}`]);
  } else {
  ...
  chatService.errorLine(`Error during rollback: ${error.shortMessage || error.message}`);
