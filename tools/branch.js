 ...
 try {
  ...
  switch (action) {
   case 'list': {
    ...
    const { stdout } = await fileSystem.executeCommand(["git", "branch", "-a"]);
   ...
   case 'create':
    ...
    chatService.systemLine(`Successfully created and switched to branch: ${branchName}`);
   ...
   case 'switch':
    ...
    chatService.systemLine(`Successfully switched to branch: ${branchName}`);
   ...
   case 'current': {
    // Show current branch
    const {stdout: currentBranch} = await fileSystem.executeCommand(["git", "branch", "--show-current"]);
   ...
   default: {
    ...
    const {stdout: currentBranchDefault} = await fileSystem.executeCommand(["git", "branch", "--show-current"]);
    const {stdout: branches} = await fileSystem.executeCommand(["git", "branch"]);

    chatService.systemLine(`Current branch: ${currentBranchDefault.trim()}`);
  ...
  chatService.errorLine(`Error with git branch operation: ${error.shortMessage || error.message}`);
  return `Branch operation failed: ${error.shortMessage || error.message}`;
 }
}

export const description = "Manages git branches - list, create, switch, or delete branches.";
