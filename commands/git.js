...
export const description = "/git <commit|rollback|branch> [options] - Git operations. Use 'commit' to commit changes, 'rollback [position]' to rollback by [position] commits (default: 1), 'branch [options]' for branch management.";
 ...
 switch (action) {
  ...
  case 'rollback': {
   ...
   if (args[1]) {
    ...
    if (!isNaN(parsed) && parsed > 0) {
     ...
     chatService.errorLine(`Invalid rollback position: "${args[1]}". Must be a positive integer.`);
     return;
    }
   }
  ...
  case 'branch': {
   ...
   if (args.length === 1) {
    ...
    if (['list', 'current', 'create', 'switch', 'delete'].includes(branchAction)) {
     branchArgs.action = branchAction;
     if (branchName) {
      branchArgs.branchName = branchName;
     }
    } else {
     chatService.errorLine(`Invalid branch action: "${branchAction}". Valid actions are: list, current, create, switch, delete`);
     return;
    }
   }
  ...
  default:
   chatService.errorLine(`Unknown git action: "${action}". Use 'commit', 'rollback', or 'branch'.`);
   break;
 }
}
